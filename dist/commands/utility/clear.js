import { PermissionFlagsBits, SlashCommandBuilder, SlashCommandChannelOption, SlashCommandNumberOption, TextChannel, } from "discord.js";
...
export default {
    data: new SlashCommandBuilder()
        .setName("clear")
        .setDescription("Supprime les derniers messages du canal spécifié (ou du canal courant sinon)")
    ...
    async execute(interaction) {
        ...
        try {
            ...
            await interaction.editReply(`Vous avez bien supprimé ${deletedMessages.size} du channel ${channel.name}`);
        }
        catch (error) {
            console.error(error);
            await interaction.editReply(`Erreur lors de la suppression, il n'est pas possible de supprimer les messages plus vieux que 14 jours (erreur: ${error})`);
        }
    },
};
